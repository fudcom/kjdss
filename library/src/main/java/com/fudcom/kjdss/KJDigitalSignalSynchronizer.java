...
public class KJDigitalSignalSynchronizer {
	...
	public synchronized void add( KJDigitalSignalProcessor pSignalProcessor ) {
		...
		if ( synchronizer != null ) {
			pSignalProcessor.initialize( sampleSize, audioFormat );
		}
	...
	public Normalizer getNormalizer() {
		...
		if ( normalizer == null ) {
			synchronized( this ) {
				if( normalizer == null ) {
					normalizer = new Normalizer( audioFormat );
				}
			}
		}
	...
	public synchronized void start( SourceDataLine pSdl ) {
		...
		synchronizer = new SourceDataLineSynchronizer( fps, frrhc );
	...
	/**
	 * Start monitoring the specified TargetDataLine.
	 * 
	 * @param pTdl A TargetDataLine.
	 */
	public synchronized void start( TargetDataLine pTdl ) {
		
		start( pTdl.getFormat() );

		dataLine = pTdl;
		
		int wBs = pTdl.getBufferSize();
		
		wBs += ( wBs % pTdl.getFormat().getFrameSize() );
		
		audioDataBuffer = new byte[ wBs ];
		
		// -- Create a new synchronizer...
		synchronizer = new TargetDataLineSynchronizer( fps, frrhc );
	...
	protected synchronized void start( AudioFormat pFormat ) {
		...
		if ( synchronizer == null ) {
			...
			// -- Calculate sample size depending on desired frame-rate, then round to the nearest sample.
			sampleSize = (int)( Math.round( pFormat.getFrameRate() / (float)fps ) );
			sampleSize += ( sampleSize % pFormat.getFrameSize() );
			...
			for( KJDigitalSignalProcessor wDsp : dsps ) {
				...
				try {
					wDsp.initialize( sampleSize, audioFormat );
				} catch( Exception pEx ) {
					pEx.printStackTrace();
				}
...
//				System.out.println( "ABW: " +  );
			...
			if ( dataLine instanceof SourceDataLine ) {
				( (SourceDataLine)dataLine ).write( pAudioData, o, wWl );
			}
...
//			}
	...
	private interface Synchronizer extends Runnable {
	...
	private class SourceDataLineSynchronizer implements Synchronizer {
		...
		public SourceDataLineSynchronizer( int pFps, int pFrrhc ) {
		...
		private int calculateSamplePosition() {
			...
			if ( DEBUG_MODE ) {
				debugInfo.put( "FP", dataLine.getLongFramePosition() );
				debugInfo.put( "DP", (long)( dataLine.getLongFramePosition() * frameSize ) % (long)( audioDataBuffer.length ) );
			}
			
			return (int)( (long)( dataLine.getLongFramePosition() * frameSize ) % (long)( audioDataBuffer.length ) );
	...
	private class TargetDataLineSynchronizer implements Synchronizer {
		...
		public TargetDataLineSynchronizer( int pFps, int pFrrhc ) {
...
//			}
			...
			int wCp = position - sampleSize;
			
			wCp += ( wCp % frameSize );
			
			if ( wCp < 0 ) {
				wCp = audioDataBuffer.length - wCp;
			} 
